...
// MetrixParser handle
type MetrixParser struct {
	*btc.BitcoinParser
}

// NewMetrixParser returns new DashParser instance
func NewMetrixParser(params *chaincfg.Params, c *btc.Configuration) *MetrixParser {
	return &MetrixParser{
		BitcoinParser: btc.NewBitcoinParser(params, c),
	}
}

// GetChainParams contains network parameters for the main Metrix network,
// the regression test Metrix network, the test Metrix network and
// the simulation test Metrix network, in this order
...
func (p *MetrixParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
...
func (p *MetrixParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
